...
class MonitorInfoCommand(
  ...
  override fun register(manager: CommandManager<Sender>) {
    ...
    monitorCommand(
      manager,
      path = listOf("config", "dump"),
      permission = "shard.monitor.config",
      playerOnly = false,
    ) {
      handler(this@MonitorInfoCommand::dumpConfig)
    }
  }
  ...
  private fun dumpConfig(context: CommandContext<Sender>) {
    val nativeSender = context.sender().nativeSender
    val target = File(plugin.dataFolder, "monitor.reference.yml")
    runCatching {
        val resource = plugin.getResource("monitor.yml") ?: error("bundled monitor.yml is missing")
        resource.use { input -> target.outputStream().use(input::copyTo) }
      }
      .onSuccess {
        MessageUtil.sendMessage(nativeSender, Message.MONITOR_CONFIG_DUMPED, "file", target.name)
      }
      .onFailure { error ->
        MessageUtil.sendMessage(
          nativeSender,
          Message.MONITOR_CONFIG_DUMP_FAILED,
          "error",
