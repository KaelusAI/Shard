...
class AiParamsStore(private val plugin: Shard) {
  private val file = File(plugin.dataFolder, FILE_NAME)

  fun readSequence(): Int? = read("sequence")

  fun readStep(): Int? = read("step")
  ...
  @Synchronized
  fun write(sequence: Int, step: Int) {
    ...
    try {
      ...
      node
        .node("_note")
        .set("Machine-managed AI window params from the inference server. Edit ai.* in config.yml.")
  ...
  private companion object {
    const val FILE_NAME = "ai-params.yml"
  }
}
